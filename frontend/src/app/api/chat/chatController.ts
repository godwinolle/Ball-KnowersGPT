...
import knowledgeBase from "../knowledgeBase/easyPlayers.json";
    ...
    if (responseMessage.tool_calls) {
        for (const toolCall of responseMessage.tool_calls) {
            ...
            if (toolCall.type !== 'function') {
                continue; // If it's not a function tool, skip custom tool calls
            }
...
const instructionsToKnowledgeBase = () => {
    const parsedKnowledgeBase = JSON.stringify(knowledgeBase) 
